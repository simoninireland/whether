# Reporter for uploading a report using MQTT
#
...
import json
import paho_mqtt
from whether import Reporter


class MQTT(Reporter):
    '''Repoorter for reporting data to an MQTT channel.

    The settings should include:

    - host: the MQTT server name or IP address
    - port: the MQTT port (defaults to 1883)
    - topic: the topic for reporting
    - discovery-topic: the topic pattern for announcing values

    The reporter will pick up MQTT_USERNAME and MQTT_PASSWORD from
    the environment to authenticate against the server. These *cannot*
    be specified in settings.

    The topic pattern can include variable expansions for:

    - {channel}: the aggregated data channel
    - {id}: the aggregator id
    ...
    def __init__(self, settings):
        ...
        self._server = settings.get('host')
        self._port = settings.get('port', 1883)
        self._topic = settings.get('topic')
        self._discovery_topic = settings.get('discovery_topic')

        self._username = environ['MQTT_USERNAME']
        self._password = environ['MQTT_PASSWORD']
        self._payload = []
